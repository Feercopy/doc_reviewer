...
- [x] Harden IC Review against provider outputs that are structurally correct
  ...
  reconciles unsupported compact verdicts to `UNKNOWN`, keeps enum/const fields
  strict, and still trims overlong strings. Added focused normalization
  coverage and an end-to-end IC Review regression where short synthesis fields
  no longer fail the whole run with
...
- [x] Replace the production analysis Summary tab with the repository
  ...
  existing analysis structured result JSON, generated by the worker from Gate
  Challenger and IC Review outputs in both Russian and English, and exposed via
  dedicated API endpoints without a database migration. The worker image now
  includes root `skills/` so `skills/new-summary/SKILL.md` is available in
  production builds. Verified focused API tests (`28 passed`), focused worker
  Summary tests (`12 passed`), focused web tests (`27 passed`), production
  Next.js build, Python compilation, Compose config, and `git diff --check`.
