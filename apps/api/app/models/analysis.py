...
class AnalysisCheckRun(Base):
    __tablename__ = "analysis_check_runs"
    __table_args__ = (
        Index("ix_analysis_check_runs_analysis_created_at", "analysis_id", "created_at"),
        Index("ix_analysis_check_runs_status_created_at", "status", "created_at"),
    )
    ...
    structured_output: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    legacy_output: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    raw_output: Mapped[str | None] = mapped_column(Text, nullable=True)
    error_message: Mapped[str | None] = mapped_column(Text, nullable=True)
    latency_ms: Mapped[int | None] = mapped_column(Integer, nullable=True)
    input_tokens: Mapped[int | None] = mapped_column(Integer, nullable=True)
    ...
    uploaded_workbook_metadata: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)
    started_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)


class AnalysisCheckStep(Base):
    __tablename__ = "analysis_check_steps"
    __table_args__ = (
        Index("ix_analysis_check_steps_run_step_name", "check_run_id", "step_name"),
    )
    ...
    status: Mapped[str] = mapped_column(String, nullable=False)
    prompt_fingerprint: Mapped[str | None] = mapped_column(String, nullable=True)
    prompt_artifact_path: Mapped[str | None] = mapped_column(String, nullable=True)
    raw_output: Mapped[str | None] = mapped_column(Text, nullable=True)
    structured_output: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    error_message: Mapped[str | None] = mapped_column(Text, nullable=True)
    latency_ms: Mapped[int | None] = mapped_column(Integer, nullable=True)
