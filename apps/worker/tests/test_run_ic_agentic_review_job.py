...
def test_completed_run_without_workbook_skips_spreadsheet_audit_and_persists_artifacts(tmp_path, monkeypatch):
    ...
    try:
        ...
        def update_result_short_summary_after_bilingual_request(**kwargs):
            result = (kwargs["analysis"].structured_output or {}).get("result") or {}
            state = result.get("summary_localizations")
            calls["bilingual_state_before_result_postprocessing"] = state
            calls["ic_status_before_result_postprocessing"] = kwargs["check_run"].status
        ...
        waiting_state = calls["bilingual_state_before_result_postprocessing"]
        assert waiting_state["ru"]["status"] == "waiting"
        assert waiting_state["en"]["status"] == "waiting"
        assert calls["ic_status_before_result_postprocessing"] == RunStatus.COMPLETED.value
        db.refresh(records["analysis"])
        bilingual_state = records["analysis"].structured_output["result"]["summary_localizations"]
...
def test_summary_localization_prepare_failure_does_not_downgrade_completed_ic_review(tmp_path, monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(job, "request_summary_localizations", fail_localization_prepare)
        monkeypatch.setattr(
            job.worker_logger,
            "info",
...
def test_summary_localization_marker_failure_does_not_downgrade_completed_ic_review(tmp_path, monkeypatch):
    db = _create_session()
    calls: dict[str, object] = {}
    logged_events: list[tuple[str, dict]] = []
    try:
        ...
        marker_failure_raised = False

        def commit_with_marker_failure():
            nonlocal marker_failure_raised
            marker = (records["check_run"].run_parameters or {}).get("summary_localizations_expected")
            if marker is True and not marker_failure_raised:
                marker_failure_raised = True
                raise ProgrammingError("UPDATE analysis_check_runs", {}, RuntimeError("database write failed"))
            original_commit()

        monkeypatch.setattr(db, "commit", commit_with_marker_failure)
        monkeypatch.setattr(
            job.worker_logger,
            "info",
            lambda event, extra=None: logged_events.append((event, extra or {})),
        ...
        db.refresh(records["analysis"])
        result = records["analysis"].structured_output["result"]
        assert marker_failure_raised is True
        assert records["check_run"].status == RunStatus.COMPLETED.value
        assert records["check_run"].current_stage == "completed"
...
def test_post_completion_reload_failure_does_not_downgrade_completed_ic_review(tmp_path, monkeypatch):
    db = _create_session()
    calls: dict[str, object] = {}
    logged_events: list[tuple[str, dict]] = []
    try:
        ...
        _patch_script_pipeline(monkeypatch, calls, validation_text="validation ok\n")
        original_get = db.get
        reload_failure_raised = False

        def get_with_post_completion_failure(model, identifier):
            nonlocal reload_failure_raised
            result = (records["analysis"].structured_output or {}).get("result") or {}
            marker = (records["check_run"].run_parameters or {}).get("summary_localizations_expected")
            if (
                model is AnalysisCheckRun
                and not reload_failure_raised
                and marker == "postprocessing"
                and result.get("rationale_status") == "completed"
            ):
                reload_failure_raised = True
                raise ProgrammingError("SELECT analysis_check_runs", {}, RuntimeError("database read failed"))
            return original_get(model, identifier)

        monkeypatch.setattr(db, "get", get_with_post_completion_failure)
        monkeypatch.setattr(
            job.worker_logger,
            "info",
            lambda event, extra=None: logged_events.append((event, extra or {})),
        ...
        db.refresh(records["check_run"])
        db.refresh(records["analysis"])
        assert reload_failure_raised is True
        assert records["check_run"].status == RunStatus.COMPLETED.value
        assert records["check_run"].current_stage == "completed"
        assert records["analysis"].structured_output["result"]["short_summary_status"] == "completed"
        assert records["analysis"].structured_output["result"]["rationale_status"] == "completed"
...
def test_short_compact_strings_are_repaired_before_validation(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert structured["critical_risks"] == ["Not provided in source materials."]
